...
public class SlotMachineLogic {
    ...
    // Añadir probabilidades de símbolos
    private static final double NO_REWARD_PROBABILITY = 0.7;
    private static final double[] SYMBOL_PROBABILITIES = {
        0.05, // extraspin - más difícil
        0.05, // siete
        0.05, // sandia
        0.05, // diamond
        0.05, // campana
        0.05, // apple
        0.1   // dinero - más común
        };
    ...
    public void spinWithAnimation(JLabel[] reels) {
        // Deduce the current bet from player credits at the start of the spin
        if (canBet()) {
            playerCredits -= currentBet;
        } else {
            return; // If the player cannot bet, do not start the spin
        }

        // Pre-generate random symbols with probabilities
        String[][] preGeneratedSymbols = new String[3][30];
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 30; j++) {
                preGeneratedSymbols[i][j] = getRandomSymbol();
            }
        }
    ...
    private String getRandomSymbol() {
        ...
        if (randomValue < NO_REWARD_PROBABILITY) {
            return "none"; // No reward
        }

        double cumulativeProbability = NO_REWARD_PROBABILITY;
        for (int i = 0; i < SYMBOL_PROBABILITIES.length; i++) {
            cumulativeProbability += SYMBOL_PROBABILITIES[i];
